                    ...
                    for doc_data in data:
                        for supply_data in doc_data.supply_data:
                            record_data = (
                                ...
                                doc_data.transport_number,
                                doc_data.truck_number
                            )
                            ...
                            receipt_transaction_number, comment, actual_quantity, unit_price, last_purchase_supplier, last_purchase_price, cancelled_due_to,planned_cost, transport_number, truck_number)
    ...
    async def get_buyer_orders(self, date_from: date, date_to: date, in_acceptance: bool) -> List[OrderedGoodsFromBuyersData]:
        if in_acceptance:
            ...
            from ordered_goods_from_buyers ogfb
                     left join receipts_for_ordered_goods_from_buyers rfogfb
                               on ogfb.id = rfogfb.ordered_goods_from_buyers_id
            where ogfb.in_acceptance = true
            and ogfb.is_valid = true
            and ogfb.is_printed_barcode = false
            and ogfb.acceptance_completed = false
            group by ogfb.id;
                    """
            async with self.pool.acquire() as conn:
            ...
            from ordered_goods_from_buyers ogfb
                     left join receipts_for_ordered_goods_from_buyers rfogfb
                               on ogfb.id = rfogfb.ordered_goods_from_buyers_id
            where ogfb.in_acceptance = false
            and ogfb.is_valid = true
            and ogfb.is_printed_barcode = false
            and ogfb.acceptance_completed = false 
            and ogfb.supply_date BETWEEN $1 AND $2
            group by ogfb.id;
                    """
            async with self.pool.acquire() as conn:
